...
// import { fetchProducts } from '../services/GlobalApi.js';
import dynamic from "next/dynamic";
import Chart, { data } from "./chart1";
export function Home() {
  const routes = [
    {
      pages: [
        ...
        {
          id: 4,
          svg: <svg xmlns="http://www.w3.org/2000/svg" width="37" height="24" viewBox="0 0 37 24" fill="none">
  ...
  //       const filteredProducts = fetchedData.products.map(product => ({
  //         title: product.title,
  //         brand: product.brand,
  //         price: product.price,
  //       }));
    ...
    <div className="mx-10 mt-10 ">
      ...
      <div className="mb-12 grid gap-y-10 gap-x-6 md:grid-cols-2 xl:grid-cols-4">
        {routes[0].pages.map((page) => (
          <div key={page.id} className={`${page.bg} rounded-xl min-w-0 h-28 p-6`}>
            <div className="flex justify-end">
              {page.svg}
            </div>
            <div className="w-[124px] font-lato mb-[20px]">
              <div className="text-sm font-semibold">{page.label}</div>
              <div className="text-2xl font-semibold">{page.price}</div>
            </div>
          </div>
        ))}
      </div>
          ...
          <div class="border-black/12.5 dark:bg-slate-850 dark:shadow-dark-xl shadow-xl relative z-20 flex min-w-0 flex-col break-words rounded-2xl border-0 border-solid bg-white bg-clip-border">
            ...
            <div className="">
              <svg xmlns="http://www.w3.org/2000/svg" width="881" height="98" viewBox="0 0 881 98" fill="none">
                <path d="M879 1.5C879 1.5 777 88 706.5 88C645 88 588 57.5 526.5 57.5C465 57.5 367.5 95 300.5 95C233.5 95 149.5 22.0001 95 22.0001C47.5 22.0001 1.5 96.5 1.5 96.5" stroke="#E9A0A0" stroke-width="3" stroke-linecap="round" stroke-linejoin="round" />
              </svg>
            </div>
          </div>
        </div>
      </div> */}
      <div className="mb-12 grid gap-y-10 gap-x-6 md:grid-cols xl:grid-cols ">

        <div className="border-2 border-black bg-white rounded-xl min-w-0 h-[340px] px-7 py-10">
